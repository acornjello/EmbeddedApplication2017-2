...
@app.route("/joy_video")
def challenge_video():
    return render_template('joy_video_01.html')
...
def echo2():
    detect = request.args.get('cnt', 0, type=int)
    cnt = 1
    ...
    time.sleep(0.5)
    requestNum = request.args.get('echoValue', 0, type = int)
    # faces = detect_faces('./static/' + 'img' + str(requestNum) + '.jpg')
    faces = detect_faces('./static/' + 'img' + str(requestNum) + '.jpg')
    likelihood_name = ('UNKNOWN', 'VERY_UNLIKELY', 'UNLIKELY', 'POSSIBLE',
                       'LIKELY', 'VERY_LIKELY')
